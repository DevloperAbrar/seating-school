...
const studentLookup = asyncHandler(async (req, res) => {
  ...
  const student = await prisma.student.findFirst({
    where: { enrollmentNo, schoolId: school.id, isActive: true },
    include: {
  ...
  res.json(
    new ApiResponse(200, "Student found", {
      student: {
        name: student.name,
...
const facultyLookup = asyncHandler(async (req, res) => {
  ...
  const duties = await prisma.invigilatorAssignment.findMany({
    where: { facultyId: faculty.id, schoolId: school.id },
    include: {
      room: { select: { name: true, building: true, floor: true } },
      shift: { select: { name: true, startTime: true, endTime: true, isPublished: true } },
      exam: { select: { title: true, academicYear: true, examDate: true, status: true } },
  ...
  res.json(
    new ApiResponse(200, "Faculty found", {
      faculty: {
        name: faculty.name,
        email: faculty.email,
      ...
      dutyAssignments: published.map((d) => ({
        exam: d.exam?.title,
        academicYear: d.exam?.academicYear,
        examDate: d.exam?.examDate,
        shift: d.shift?.name,
        time: `${d.shift?.startTime} – ${d.shift?.endTime}`,
        room: d.room?.name,
